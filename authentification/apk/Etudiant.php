...
class Etudiant
{
	...
	function __construct($nom,$mdp=null, $ip=null, $mac=null)
	{
		...
		// $this->mac=(is_null($mac)? getMac() : $mac );
		$this->url=getUrl();
		if($this->url=="")
		{
			$this->url="https://www.google.com";
		}
			$this->serverLDAP="127.0.0.1";
		$this->passLDAP="123456";
		$this->dnLDAP="cn=admin,dc=authentification,dc=com";
	} 
	public function display()
	{
		$resultat="<form method='post' action='/authentification/Admin/delete'>
					<tr>
		 ...
		 echo $resultat;
		// <a onclick='confirm22()' href='/authentification/Admin/delete?ip=$this->ip'> Arreter</a>
	}
        ...
        if(ldap_bind($ldap_con, $ldap_dn, $ldap_password))
        {
            ...
            if (!empty($entries[0]['cn'][0]) && $entries[0]['cn'][0]==$this->pseudo) {
                ...
                else
                {
					...
					header('Location:/authentification/Acceuil/index');
